...
from db.demanders import Demanders
...
def close_job(job_dic,msg_id,from_chat_id):
    try:
        ...
        register_job(job_dic,msg_id,from_chat_id,'closed')
...
def register_job(job_dict,msg_id,from_chat_id,status='open'):
    try:
        ...
        job = Post(message_id=msg_id,
              job_title=job_dict['title'],job_type=job_dict['type'],
              description=job_dict['description'],company=job_dict['company'] if job_dict['company'] else "",
              from_chat_id=from_chat_id,category=job_type,job_status=status)
        job.save()
    return job
...
def fwd_msg_handler(update, context):
    ...
    #"https://t.me/freelance_ethio/{update.message.message_id}"
        ...
        if rgx_dict['status'].lower() == 'open':
            ...
            close_job(rgx_dict,msg_id,from_chat_id)
...
def register_user(user_id, username, job_type):
    ...
    num = Demanders.objects.get(category=job_type)
    num.demanders+=1
    num.save()
...
def button(update, context) -> None:
    ...
    if query.data == 'register':
        ...
        get_jobs_by_status(update,context,user_id,'closed')
        ...
        try:
            ...
            user = Seeker.objects.get(user_id=user_id)
            num = Demanders.objects.get(category=user.job_type)
            num.demanders-=1
            num.save()
